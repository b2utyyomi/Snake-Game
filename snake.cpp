...
void print_information(HANDLE hOut,struct Snake *snake,struct Food *food)
{
     ...
     printf("level : %d",snake->cnt/5+1);    ///打印游戏等级

     gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+3);
     printf("score : %d",10*snake->cnt);    ///打印游戏得分

     gotoxy(hOut,FrameX+2*Frame_width+3,FrameY+5);
     printf("eat food : %d",snake->cnt);    ///打印产生食物个数
...
void make_frame()
{
     ...
     printf("╔ ");
     gotoxy(hOut,FrameX+2*Frame_width,FrameY);
     printf("╗");
     gotoxy(hOut,FrameX,FrameY+Frame_height);
     printf("╚");
     gotoxy(hOut,FrameX+2*Frame_width,FrameY+Frame_height);
     printf("╝");
     for(i=2;i<2*Frame_width;i+=2)///这个图形啊 是两个长度的 然后i从1开始的话 就把角覆盖了
     {
      gotoxy(hOut,FrameX+i,FrameY);///  为什么是2*Frane_width呢 因为图形║是图形═长度的2倍
      printf("═");         ///打印上横框(sougou)
     }
     ...
     for(i=1;i<Frame_height;i++)///竖框呢 i就该从一开始 从2开始的话就断了 这是图形决定的啊
     {
     ...
     {
      gotoxy(hOut,FrameX+2*Frame_width,FrameY+i);
      printf("║");         ///打印右竖框
     }
     gotoxy(hOut,FrameX+Frame_width-5,FrameY-2);   ///打印游戏名称
     Set_Color_Red();        ///设置蛇为红色
     printf("贪吃蛇游戏");
}
/******结束菜单*******************************************************************/
void over_game()
...
void eat_food(HANDLE hOut,struct Snake *snake,struct Food *food)
{
     ...
     { /// 如果蛇头位置与食物位置相同，吃食物
          ...
          for(i=snake->length-1;i>=1;i--)///啊啊啊啊 想错了 snake[snake->length-1]是头
...
{ /// 当蛇头碰到自身时，蛇死 ，返回值为0
     for(i=0;i<snake->length-1;i++)
     {
          ...
          if(snake->y[snake->length-1] == FrameY || snake->y[snake->length-1] == FrameY+Frame_height)
            return 0;
          if(snake->x[snake->length-1] == FrameX || snake->x[snake->length-1] == FrameY+2*Frame_height)
            return 0;
     }
     return 1;
}
...
void start_game()
{
     ...
     while(1)
     {
      ...
      if(j==0)
      {
        Sleep(snake->speed);      ///延迟时间  为了让蛇速度慢下来
      }
      else
      {
        Sleep(10);
